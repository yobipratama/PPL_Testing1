                  ...
                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                    ...
                    @foreach ($buku as $key)
                      <tr>
                        ...
                        <td>{{ $key -> nama_pengarang }}</td>
                        <td>{{ $key -> nama_penerbit }}</td>
                        <td>{{ $key -> ketebalan }}</td>
                        <td>{{ $key -> tahun_terbit }}</td>
                        <td>{{ $key -> jumlah_buku }}</td>
                        <td><img width="200px" src="{{ asset('storage/'.$key->image) }}"></td>
                        <td>
                            <form action="{{ route('buku.destroy',['buku'=>$key->id_buku]) }}" method="POST">

                                <a class="btn btn-info" href="{{ route('buku.show',$key->id_buku) }}">Show</a>
                                <a class="btn btn-primary" href="{{ route('buku.edit',$key->id_buku) }}">Edit</a>
                                @csrf
                                @method('DELETE')
